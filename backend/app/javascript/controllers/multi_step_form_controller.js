...
export default class extends Controller {
  ...
  render() {
    ...
    if (this.hasConnectorLineTarget) {
      this.connectorLineTarget.classList.toggle(
        "bg-primary-500",
        this.current >= 1,
      );
      this.connectorLineTarget.classList.toggle(
  ...
  hoursChanged() {
    ...
    this.photoRequiredLabelTarget.textContent = required
      ? "(required)"
      : "(optional)";
  ...
  beforeSubmit(event) {
    ...
    const hours = parseFloat(this.hoursInputTarget.value) || 0;
    const hasPhotos =
      this.photosInputTarget.files && this.photosInputTarget.files.length > 0;
